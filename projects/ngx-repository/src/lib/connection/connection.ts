...
export abstract class Connection<RC, RQ, RS> {
  ...
  protected abstract getRepositoryInstance<T, K, Q extends Query<K> = null>(resourceType: new(...args: any) => T): AbstractRepository<T, K, Q, RC, RQ, RS>;

  public getRepository<T, K, Q extends Query<K> = null>(resourceType: new(...args: any) => T): AbstractRepository<T, K, Q, RC, RQ, RS> {
    if (!Reflect.hasMetadata(this.resourceContextKey, resourceType)) {
      throw new Error(`${resourceType} is not a valid resource.`);
    }

    const repository: AbstractRepository<T, K, Q, RC, RQ, RS> = this.getRepositoryInstance(resourceType);
