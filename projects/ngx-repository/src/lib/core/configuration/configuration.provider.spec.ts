...
describe('ConfigurationProvider', () => {
  ...
  describe('#getConfiguration', () => {
    ...
    it('should throw an error if searched configuration is not find', () => {
      ...
      expect(() => provider.getConfiguration('myProperty', ['foo', 'bar'])).toThrowError(`Unable to find configuration 'myProperty' (${ JSON.stringify(paths) })`);
    });
    ...
    it('should return the configuration', () => {
      ...
      expect(provider.getConfiguration('myProperty', ['foo', 'bar'])).toBe(configuration);
      expect(provider.findConfiguration).toHaveBeenCalledTimes(1);
      expect(provider.findConfiguration).toHaveBeenCalledWith('myProperty', ['foo', 'bar']);
    });
  });
  ...
  describe('#findConfiguration', () => {
    ...
    it('should return null path or property not exist in params', () => {
      expect(provider.findConfiguration('myProperty', ['foo', 'bar'])).toBeNull();
    });

    it('should return the property value if property exist in  root params', () => {
      params[`myProperty`] = 'toto';
      expect(provider.findConfiguration('myProperty', ['foo', 'bar'])).toEqual('toto');
    });
