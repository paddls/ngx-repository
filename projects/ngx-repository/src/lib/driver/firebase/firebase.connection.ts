...
@Injectable()
export class FirebaseConnection extends Connection<FirebaseResourceContext, any, Observable<any>> {
  ...
  protected makeToken<T, K>(resourceType: new(...args: any) => T): InjectionToken<FirebaseRepository<T, K>> {
    return new InjectionToken<FirebaseRepository<T, K>>(`${resourceType.name}_Repository$`);
  }
}
